...
const STORAGE_KEYS = {
  TRACKED: 'appfiliate_install_tracked',
  MATCHED: 'appfiliate_matched',
  ATTRIBUTION_ID: 'appfiliate_attribution_id',
} as const;
...
class Appfiliate {
  ...
  private _apiBase = 'https://us-central1-appfiliate-5a18b.cloudfunctions.net/api';
  ...
  async trackInstall(): Promise<AttributionResult> {
    if (!this._configured) {
      console.warn('[Appfiliate] Error: call appfiliate.configure() before trackInstall()');
    ...
    const payload: Record<string, unknown> = {
      ...
      sdk_version: Appfiliate.SDK_VERSION,
    };
    ...
    try {
      ...
      console.log(
        `[Appfiliate] Install attribution: matched=${result.matched}, confidence=${result.confidence}, method=${result.method}`
      );

      return result;
    } catch (error) {
      console.warn('[Appfiliate] Attribution error:', error);
  ...
  async trackPurchase(opts: TrackPurchaseOptions): Promise<void> {
    if (!this._configured) {
      console.warn('[Appfiliate] Error: call appfiliate.configure() before trackPurchase()');
      return;
    }

    const attributionId = this._attributionId ?? (await AsyncStorage.getItem(STORAGE_KEYS.ATTRIBUTION_ID));
    if (!attributionId) {
      console.warn('[Appfiliate] No attribution ID found. Install may not have been attributed.');
      return;
    }
    ...
    try {
      ...
      console.log(`[Appfiliate] Purchase tracked: ${opts.productId} ${opts.revenue} ${opts.currency ?? 'USD'}`);
    } catch (error) {
      console.warn('[Appfiliate] Purchase tracking error:', error);
    }
  }
  ...
  async setUserId(userId: string): Promise<void> {
    if (!this._configured) {
      console.warn('[Appfiliate] Error: call appfiliate.configure() before setUserId()');
      return;
    }

    const attributionId = this._attributionId ?? (await AsyncStorage.getItem(STORAGE_KEYS.ATTRIBUTION_ID));
    if (!attributionId) {
      console.warn('[Appfiliate] No attribution ID found. Call trackInstall() before setUserId().');
      return;
    }
    ...
    try {
      ...
      console.log(`[Appfiliate] User ID linked: ${userId}`);
    } catch (error) {
      console.warn('[Appfiliate] setUserId error:', error);
    }
  }

  /**
   * Whether this install was attributed to a creator.
...
/** Singleton Appfiliate instance. */
export const appfiliate = new Appfiliate();
